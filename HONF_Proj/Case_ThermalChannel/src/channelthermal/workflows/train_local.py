"""Train the ChannelThermal Stage-A local module surrogate.

This case-owned implementation is dispatched by the project-level ``train.py``.
The config selects standalone local data, global-alignment modules, or a
weighted mixture. One training-fitted normalizer is shared by all sources and
validation splits. Outputs are self-contained checkpoints, the resolved config,
a CSV loss history, and a loss curve.
"""
...
def _auto_int(value: Any, fallback: int) -> int:
    """Resolve an integer or the sentinel ``auto`` against a dataset value."""
...
class MixedLocalDataset(Dataset):
    """Concatenate Stage-A sources while retaining source sampling weights."""

    def __init__(self, datasets: Sequence[Dataset], source_weights: Optional[Sequence[float]] = None):
        """Initialize MixedLocalDataset and its required state."""
        ...
        weights = list(source_weights or [1.0] * len(self.datasets))
        if len(weights) != len(self.datasets) or any(float(value) < 0.0 for value in weights):
            raise ValueError("source_weights must contain one non-negative value per dataset.")
        if not any(float(value) > 0.0 for value in weights):
            raise ValueError("At least one source weight must be positive.")
        self.source_weights = [float(value) for value in weights]
    ...
    def sampling_weights(self) -> torch.Tensor:
        """Return per-record weights whose total mass matches each source weight."""

        pieces = [
            torch.full((length,), weight / max(length, 1), dtype=torch.double)
            for length, weight in zip(self.lengths, self.source_weights)
        ]
...
def build_stage_a_dataset(
    ...
    *,
    ...
    """Construct one Stage-A source view with an explicitly supplied transform."""

    normalize_inputs = bool(dataset_cfg.get("normalize_inputs", False)) if normalize_inputs is None else bool(normalize_inputs)
    normalize_targets = bool(dataset_cfg.get("normalize_targets", False)) if normalize_targets is None else bool(normalize_targets)
    source = str(source).lower()
    if source == "local":
        return LocalModuleDataset(
            dataset_cfg.get("packed_h5_path", "./Case_ThermalChannel/Dataset/links/thermal_disk_local_v1.h5"),
            split=split,
    ...
    if source == "global_alignment":
        return GlobalModuleAlignmentDataset(
            dataset_cfg.get("global_alignment_packed_h5_path", "./Case_ThermalChannel/Dataset/links/thermal_channel_global_v1.h5"),
            split=split,
            normalize_inputs=normalize_inputs,
            normalize_targets=normalize_targets,
            include_grid=False,
    ...
    if source == "mixed":
        ...
        return MixedLocalDataset(
            [local, global_align],
            source_weights=[
                float(dataset_cfg.get("local_synthetic_weight", 1.0)),
                float(dataset_cfg.get("global_alignment_weight", 1.0)),
            ],
        )
    raise ValueError("dataset.source must be 'local', 'global_alignment', or 'mixed'.")


def _dataset_parts(dataset: Dataset) -> list[Dataset]:
    """Return the physical child datasets used to fit a shared normalizer."""

    return list(dataset.datasets) if isinstance(dataset, MixedLocalDataset) else [dataset]


def _apply_normalizer(dataset: Dataset, normalizer: H5Normalizer, *, inputs: bool, targets: bool) -> None:
    """Apply one immutable Stage-A transform to a dataset and all mixture children."""

    for part in _dataset_parts(dataset):
        part.normalizer = normalizer
        part.normalize_inputs = bool(inputs)
        part.normalize_targets = bool(targets)
...
def sanitize_run_suffix(value: Any) -> str:
    """Convert an optional run suffix to a filesystem-safe name."""
...
def run_epoch(
    ...
    *,
    ...
    """Run one Stage-A train/validation epoch and average its loss terms."""
...
def save_checkpoint(
    ...
    *,
    ...
    """Atomically save Stage-A weights, config, feature schema, and transform."""
    ...
    payload = {
            ...
            "dataset_schema": dataset_cfg.get("dataset_schema"),
            "dataset_fingerprint": dataset_cfg.get("dataset_fingerprint"),
            "global_alignment_dataset_id": dataset_cfg.get("global_alignment_dataset_id"),
            "global_alignment_dataset_schema": dataset_cfg.get("global_alignment_dataset_schema"),
            "global_alignment_dataset_fingerprint": dataset_cfg.get("global_alignment_dataset_fingerprint"),
            ...
            # These states make a resumed sampler/dropout stream a true
            # continuation rather than a fresh run with reused weights.
            "rng_state": {
...
def _restore_rng_state(payload: Dict[str, Any]) -> None:
    """Restore optional RNG state written by current Stage-A checkpoints."""

    state = dict(payload.get("rng_state") or {})
...
def _validate_resume_checkpoint(
    ...
    *,
    ...
    """Reject an incompatible Stage-A resume before changing model state."""
    ...
    if saved_config and saved_config != model_config.to_dict():
        raise ValueError("Resume checkpoint Stage-A architecture does not match the resolved configuration.")
    expected_features = {
        "module_param_names": list(dataset.module_param_names),
        "port_input_feature_names": list(dataset.port_input_feature_names),
        "interface_target_names": list(dataset.interface_target_names),
    }
    saved_features = dict(checkpoint.get("dataset_feature_names") or {})
    if saved_features and saved_features != expected_features:
        raise ValueError("Resume checkpoint Stage-A feature schema does not match the selected dataset.")
    ...
    if saved_fingerprint and expected_fingerprint and saved_fingerprint != expected_fingerprint:
        raise ValueError("Resume checkpoint Stage-A dataset fingerprint does not match the selected dataset.")
    saved_stats = dict(checkpoint.get("local_normalization_stats") or {})
    current_stats = dict(getattr(getattr(dataset, "normalizer", None), "stats", {}))
    for name, saved_value in saved_stats.items():
        if name not in current_stats or not np.allclose(
            np.asarray(saved_value), np.asarray(current_stats[name]), rtol=1.0e-7, atol=1.0e-7
        ):
            raise ValueError(f"Resume checkpoint normalization mismatch for {name!r}.")


def _repair_history_for_append(path: Path) -> None:
    """Remove interrupted NUL/partial tails before appending a resumed run."""
...
def load_init_checkpoint_if_requested(model: LocalModuleSurrogate, init_path: Optional[str], device: torch.device) -> None:
    """Load init checkpoint if requested."""
...
def run_from_config(
    ...
    *,
    ...
    """Train the ThermalChannel local module from a composed case config."""
    ...
    # Build raw training views first, fit exactly one transform, then reuse it
    # for all sources and validation data. An empty injected normalizer avoids
    # fitting split-specific statistics inside GlobalModuleAlignmentDataset.
    train_dataset = build_stage_a_dataset(
        dataset_cfg,
        split=train_split,
    ...
    shared_normalizer = fit_local_normalizer(_dataset_parts(train_dataset))
    _apply_normalizer(
        train_dataset,
        shared_normalizer,
        inputs=bool(dataset_cfg.get("normalize_inputs", False)),
        targets=bool(dataset_cfg.get("normalize_targets", False)),
    )
    ...
    val_dataset = build_stage_a_dataset(
        dataset_cfg,
        split=val_split,
        source=dataset_source,
    ...
    if len(val_dataset) == 0:
        if bool(dataset_cfg.get("allow_train_as_validation", False)):
            print("[warning] Stage-A validation split is empty; explicitly reusing training data.")
            val_dataset = train_dataset
        else:
            raise RuntimeError(
                "Stage-A validation split is empty. Select a non-empty split or set "
                "local_modules.<id>.allow_train_as_validation=true explicitly."
            )
    ...
    resume_value = getattr(args, "resume_checkpoint", None)
    if init_checkpoint and resume_value:
        raise ValueError("Use either an initialization checkpoint or --resume-checkpoint, not both.")
    if not resume_value:
    ...
    train_sampler = None
    if isinstance(train_dataset, MixedLocalDataset):
    ...
    saved_root = ensure_dir(resolve_demo_path(cfg.get("paths", {}).get("saved_model_dir", "./Trained_Results/ThermalChannel/Local_Module_Runs/thermal_disk")))
    ...
    if resume_checkpoint is not None:
        _repair_history_for_append(history_path)
    else:
    ...
    if resume_checkpoint is not None:
        ...
        if not optimizer_state:
            print("[resume] checkpoint has no optimizer state; continuing with a fresh optimizer.")
    ...
    write_json(
        run_dir / "summary.json",
        {
            ...
            "run_dir": str(run_dir),
            "best_val_loss_total": best_metric,
            "epochs": epochs,
            "last_completed_epoch": max(start_epoch - 1, epochs),
