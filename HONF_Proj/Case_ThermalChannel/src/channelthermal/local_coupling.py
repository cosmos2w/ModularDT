...
statistics. global restores the proven ChannelThermal physical corrections:
...
from honf_runtime.compat import MLP, FourierEncoder, load_trusted_checkpoint, resolve_demo_path, safe_std_torch, strip_module_prefix
...
def build_local_module_params_from_global(
    ...
    """Assemble ``[B,M,7]`` heat/material/interface summaries for Stage A."""
...
class PortConditionHead(nn.Module):
    ...
    def __init__(self, hidden_dim: int, dropout: float, use_layer_norm: bool):
        """Initialize PortConditionHead and its required state."""
    ...
    def fixed_theta_tokens(self, ntheta: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        """Return evenly spaced ``[P,3]`` angle/cosine/sine tokens."""
    ...
    def forward(
        ...
        *,
        ...
        """Predict ``T_env`` and positive ``h`` for ``[B,M,P,5]`` port tokens."""
...
class FluxCorrectionHead(nn.Module):
    ...
    def __init__(self, hidden_dim: int, local_surrogate_latent_dim: int, dropout: float, use_layer_norm: bool):
        """Initialize FluxCorrectionHead and its required state."""
    ...
    def forward(
        ...
        """Predict a residual normal-flux correction at each ``[B,M,P]`` port."""
...
class PortRefinementHead(nn.Module):
    ...
    def __init__(self, hidden_dim: int, dropout: float, use_layer_norm: bool):
        """Initialize PortRefinementHead and its required state."""
    ...
    def forward(
        ...
        """Refine port ``T_env``/``h`` after probing the provisional global field."""
...
class LocalSurrogateCoupling(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize LocalSurrogateCoupling and its required state."""
        ...
        if self.local_surrogate_flux_mode not in {"surrogate", "physics_from_port", "corrected_physics", "blend"}:
            raise ValueError(f"Unknown local_surrogate_flux_mode={self.local_surrogate_flux_mode!r}.")
        if not 0.0 <= self.local_surrogate_flux_blend_alpha <= 1.0:
    ...
    def has_local_surrogate(self) -> bool:
        """Return whether a Stage-A model has been attached."""
    ...
    def attach_from_checkpoint(self, checkpoint_path: str | Path, *, freeze: bool = True, map_location: Any = "cpu") -> Dict[str, Any]:
        """Load a trusted Stage-A checkpoint, its normalization, and optionally freeze it."""
    ...
    def set_global_target_normalization(self, stats: Optional[Dict[str, Any]], *, normalize_targets: bool) -> None:
        """Set global target normalization."""
    ...
    def train(self, mode: bool = True) -> "LocalSurrogateCoupling":
        """Set training mode while keeping a frozen Stage-A surrogate in evaluation mode."""
    ...
    def _set_buffer_from_stat(self, name: str, stats: Optional[Dict[str, Any]], key: str, *alternate_keys: str) -> None:
        """Copy one available normalization statistic into a registered buffer."""
    ...
    def _normalize_with_stats(self, values: torch.Tensor, mean: torch.Tensor, std: torch.Tensor) -> torch.Tensor:
        """Apply broadcast-safe z-score normalization when statistics exist."""
    ...
    def _denormalize_with_stats(self, values: torch.Tensor, mean: torch.Tensor, std: torch.Tensor) -> torch.Tensor:
        """Undo broadcast-safe z-score normalization when statistics exist."""
    ...
    def choose_local_ports(
        ...
        *,
        ...
        """Select predicted, teacher, or convexly mixed physical port tokens."""

        mode = str(mode).lower()
        if mode not in {"predicted", "teacher", "mixed"}:
            raise ValueError("mode must be 'predicted', 'teacher', or 'mixed'.")
        if mode in {"teacher", "mixed"} and teacher_port_tokens is None:
            raise ValueError(f"mode={mode!r} requires teacher_port_tokens.")
        ratio = float(mixed_teacher_ratio)
        if not 0.0 <= ratio <= 1.0:
            raise ValueError("mixed_teacher_ratio must be in [0, 1].")
        if mode == "teacher":
    ...
    def local_module_params_for_ports(
        ...
        """Optionally refresh local parameter summaries from selected ports."""
    ...
    def call_local_surrogate(
        ...
        """Evaluate Stage A for active modules and scatter results to padded slots."""
        ...
        active = module_present.reshape(-1) > 0.5
        active_indices = torch.nonzero(active, as_tuple=False).reshape(-1)
        query_count = 0 if flat_query is None else int(flat_query.shape[-2])
        latent_dim = int(self.local_surrogate.config.latent_dim)
        interface_dim = int(self.local_surrogate.config.interface_target_dim)
        if active_indices.numel() == 0:
            internal = module_params.new_zeros(batch, num_modules, query_count, 1)
            interface = module_params.new_zeros(batch, num_modules, ntheta, interface_dim)
            latent = module_params.new_zeros(batch, num_modules, latent_dim)
            return {
                "internal_temperature": internal,
                "interface_pred": interface,
                "module_response_latent": latent,
            }
        active_query = None if flat_query is None else flat_query.index_select(0, active_indices)
        out = self.local_surrogate(
            flat_params.index_select(0, active_indices),
            flat_ports.index_select(0, active_indices),
            active_query,
        )
        internal_active = out["internal_temperature"]
        interface_active = out["interface_pred"]
        if self.local_surrogate_normalize_targets:
            internal_active = self._denormalize_with_stats(internal_active, self.local_internal_temperature_mean, self.local_internal_temperature_std)
            interface_active = self._denormalize_with_stats(interface_active, self.local_interface_targets_mean, self.local_interface_targets_std)
        if self.global_normalize_targets:
            internal_active = self._normalize_with_stats(internal_active, self.global_internal_temperature_mean, self.global_internal_temperature_std)
            interface_active = self._normalize_with_stats(interface_active, self.global_interface_target_mean, self.global_interface_target_std)
        flat_internal = module_params.new_zeros(batch * num_modules, query_count, 1)
        flat_interface = module_params.new_zeros(batch * num_modules, ntheta, interface_dim)
        flat_latent = module_params.new_zeros(batch * num_modules, latent_dim)
        flat_internal.index_copy_(0, active_indices, internal_active)
        flat_interface.index_copy_(0, active_indices, interface_active)
        flat_latent.index_copy_(0, active_indices, out["module_response_latent"])
        internal = flat_internal.reshape(batch, num_modules, query_count, 1)
        interface = flat_interface.reshape(batch, num_modules, ntheta, interface_dim)
        latent = flat_latent.reshape(batch, num_modules, latent_dim)
        return {
            "internal_temperature": internal,
            "interface_pred": interface,
            "module_response_latent": latent,
    ...
    def local_response_summary(
        ...
        *,
        ...
        """Reduce local internal/interface fields to six physical module statistics."""
    ...
    def fuse_module_state(
        ...
        """Fuse Stage-A latent and response statistics into ``[B,M,H]`` tokens."""
