...
**Reader-recovery follow-up note:** The historical `field_main_zero` hook below wrapped every `core.decode_queries` call, including the provisional P1 outside-temperature decode used for interface refinement. Its numbers therefore measure broad decoder suppression, not isolated final P2 field-read suppression. The original measurements and names are retained; the reader-recovery study adds phase-explicit interventions. Also, the 47.2% excess stated in the KPI discussion refers to latent's pooled **relative L2**, not pooled MSE; the tabulated values are unchanged. Run-1804 continuation is deferred for the reader-only experiment described in the recovery plan.
