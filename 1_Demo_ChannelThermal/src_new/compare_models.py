"""Compare multiple ChannelThermal NewHONF checkpoints on a dataset split.

Inputs are Run_ID selectors or explicit NewHONF checkpoint paths. Outputs are
case/model metric tables, summary tables, logs, and presentation-ready figures
under Saved_Model_NewHONF/CompareModels/Run_<timestamp> by default.

Example usage:
python src_new/compare_models.py \
  --Run_ID 0004 \
  --Run_ID 0005 \
  --Run_ID 0006 \
  --label NoPretrained \
...
from _helpers.evaluation_plots import module_and_fluid_masks
from _helpers.model_utils import current_timestamp, load_trusted_checkpoint, resolve_demo_path, select_device, write_json
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare multiple ChannelThermal NewHONF checkpoints.")
    ...
    parser.add_argument("--saved-root", default="./Saved_Model_NewHONF")
...
def resolve_model_specs(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    for run_id in args.run_ids:
        ...
        if not checkpoint_path.exists() and args.checkpoint_selector == "best_predicted":
...
def load_checkpoint_for_spec(spec: Dict[str, Any], *, retries: int, retry_delay: float) -> tuple[Dict[str, Any], Path]:
    requested = Path(spec["checkpoint_path"]).resolve()
    try:
        ...
        if not str(spec.get("source", "")).startswith("Run_ID:"):
            raise
        tqdm.write(
...
def load_checkpoint_and_model_for_spec(
    ...
    *,
    ...
    if str(spec.get("source", "")).startswith("Run_ID:"):
...
def checkpoint_dataset_config(checkpoint_path: Path, *, retries: int, retry_delay: float) -> Dict[str, Any]:
    checkpoint = load_checkpoint_with_retries(checkpoint_path, retries=retries, retry_delay=retry_delay)
    return dict(checkpoint.get("train_config", {}).get("dataset", {}))
...
def hypergraph_metrics(base_row: Dict[str, Any], raw_sample: Dict[str, Any], predictions: Dict[str, Any]) -> Dict[str, Any]:
    ...
    row["has_solved_structure_targets"] = float(np.asarray(structure_targets.get("has_solved_structure_targets", [0.0])).reshape(-1)[0])
    if A_mh.size and A_eh.size:
    ...
    if "active_edge_count_target" in structure_targets:
...
def main() -> int:
    ...
    if len(raw_dataset) == 0:
        ...
        actual_split = "all"
        raw_dataset = GlobalChannelThermalDataset(dataset_path, split="all", points_per_case=1, normalize_inputs=False, normalize_targets=False, random_point_sampling=False, include_grid=True)
