...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Map ``[..., in_dim]`` features to ``[..., out_dim]``."""
...
class LazyMLP(nn.Module):
    ...
    def __init__(self, hidden_dim: int, out_dim: int, num_layers: int, dropout: float):
        """Initialize LazyMLP and its required state."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Map arbitrary-width ``[..., D]`` features to ``[..., out_dim]``."""
...
class FourierFeatures(nn.Module):
    ...
    def __init__(self, num_frequencies: int):
        """Initialize FourierFeatures and its required state."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Encode ``[..., D]`` coordinates as ``[..., D*(1+2F)]`` features."""
...
def rectangular_boundary_features(query_xy: torch.Tensor, Lx: float, Ly: float) -> torch.Tensor:
    """Return normalized position and distances to four rectangle boundaries."""
