...
Run ``python src_HONF_CL/train.py --config <json>`` from the Demo 1 directory.
The JSON file controls the reusable HONF core, ChannelThermal local coupling,
data sampling, losses, optimizer, checkpoints, and output location. 

A run writes resolved settings, CSV metrics, selected checkpoints, diagnostic loss
curves, and a compact summary.  For quick run:

python src_HONF_CL/train.py \
  --config src_HONF_CL/configs/train_global_honf_validated_core.json
...
from data.datasets import GlobalChannelThermalDataset
...
from training_tools.honf_diagnostics import HONF_DIAGNOSTIC_KEYS, compute_honf_diagnostics, organizer_regularization_loss
from training_tools.losses import weighted_field_mse
from channelthermal.config import ChannelThermalHONFConfig
from channelthermal.model import ChannelThermalHONFModel


DEFAULT_CONFIG_PATH = "./src_HONF_CL/configs/train_global_honf_template.json"
...
def main() -> int:
    ...
    saved_root = ensure_dir(resolve_demo_path(paths_cfg.get("saved_model_dir", "./Saved_Model_HONF_CL/Global")))
