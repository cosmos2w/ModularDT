...
def channel_cmap(name: str) -> str:
    """Perform the channel cmap operation used by this module."""
...
def error_metrics(prediction: np.ndarray, target: np.ndarray) -> Dict[str, float]:
    """Perform the error metrics operation used by this module."""
...
def masked_error_metrics(prediction: np.ndarray, target: np.ndarray, mask: np.ndarray) -> Dict[str, float]:
    """Perform the masked error metrics operation used by this module."""
...
def module_radius_from_sample(sample: Dict[str, Any], fallback: float = 0.45) -> float:
    """Perform the module radius from sample operation used by this module."""
...
def module_and_fluid_masks(sample: Dict[str, Any], pred_field: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
    """Perform the module and fluid masks operation used by this module."""
...
def draw_module_outlines(ax: Any, sample: Dict[str, Any], color: str = "#d9d9d9", linewidth: float = 1.0) -> None:
    """Perform the draw module outlines operation used by this module."""
...
def _local_disk_image(values: np.ndarray, local_mask: np.ndarray) -> np.ndarray:
    """Perform the local disk image operation used by this module."""
...
def composite_temperature_grid(sample: Dict[str, Any], global_temperature: np.ndarray, internal_temperature_points: np.ndarray) -> np.ndarray:
    """Perform the composite temperature grid operation used by this module."""
...
def raster_local_points(values: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Map flattened circular-module values back onto their local square grid."""

    image = np.full(mask.shape, np.nan, dtype=np.float32)
    image[np.asarray(mask, dtype=bool)] = np.asarray(values).reshape(-1)
    return image


def plot_local_internal(
    output_path: Path,
    sample: Dict[str, Any],
    pred_internal: np.ndarray,
    metrics: Dict[str, float],
) -> bool:
    """Plot Stage-A internal temperature target, prediction, and absolute error."""

    mask = sample.get("local_mask")
    if mask is None:
        return False
    target = np.asarray(sample["internal_temperature_targets"]).reshape(-1)
    prediction = np.asarray(pred_internal).reshape(-1)
    gt_image = raster_local_points(target, mask)
    pred_image = raster_local_points(prediction, mask)
    error_image = np.abs(pred_image - gt_image)
    vmin, vmax = float(np.nanmin(gt_image)), float(np.nanmax(gt_image))
    fig, axes = plt.subplots(1, 3, figsize=(10.0, 3.2), constrained_layout=True)
    panels = [
        (gt_image, "GT internal T", "inferno"),
        (pred_image, "Pred internal T", "inferno"),
        (error_image, f"Abs error\nRMSE={metrics['rmse']:.4e}", "magma"),
    ]
    for column, (image, title, cmap) in enumerate(panels):
        plotted = axes[column].imshow(
            image,
            origin="lower",
            extent=(-1, 1, -1, 1),
            cmap=cmap,
            vmin=vmin if column < 2 else None,
            vmax=vmax if column < 2 else None,
        )
        axes[column].set_title(title)
        axes[column].set_xlabel("xi")
        axes[column].set_ylabel("eta")
        fig.colorbar(plotted, ax=axes[column], fraction=0.046, pad=0.04)
    fig.savefig(str(output_path), dpi=170)
    plt.close(fig)
    return True
...
def plot_local_interface(output_path: Path, sample: Dict[str, Any], pred_interface: np.ndarray) -> None:
    """Plot Stage-A boundary inputs and predicted/target interface responses."""

    theta = np.asarray(sample["port_tokens"])[:, 0]
    target = np.asarray(sample["interface_targets"])
    roughness = np.asarray(sample.get("local_target_roughness", np.zeros((4,), dtype=np.float32)))
    ...
    h_axis = axes[0].twinx()
    h_axis.plot(theta, sample["port_tokens"][:, 4], color="#f58518", lw=1.3, label="h")
    h_axis.set_ylabel("h")
    axes[0].set_title(
    ...
    for index, axis in enumerate(axes[1:]):
        label = ["T_surface", "q_normal"][index]
        channel_metrics = error_metrics(pred_interface[:, index], target[:, index])
        axis.plot(theta, target[:, index], color="black", lw=1.8, label="GT")
        axis.plot(theta, pred_interface[:, index], color="#d95f02", lw=1.5, label="Pred")
        if "interface_targets_raw" in sample:
            axis.plot(theta, sample["interface_targets_raw"][:, index], color="#7f7f7f", lw=1.0, alpha=0.65, label="raw target")
        axis.set_ylabel(label)
        axis.set_title(
            f"{label} RMSE={channel_metrics['rmse']:.4e}, relL2={channel_metrics['relative_l2']:.4e}, "
            f"rough={float(roughness[index]):.3f}"
        )
