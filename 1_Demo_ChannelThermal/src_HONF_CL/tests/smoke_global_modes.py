...
Inputs are the bundled HONF-CL config and existing packed global HDF5
...
from common.runtime import read_json, recursive_to_device, resolve_demo_path, select_device
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Smoke-test global HONF-CL forward modes.")
    parser.add_argument("--config", type=str, default="./src_HONF_CL/configs/train_global_honf_template.json")
...
def assert_finite(name: str, value: torch.Tensor) -> None:
    """Perform the assert finite operation used by this module."""
...
def run_mode(base_cfg: dict, dataset: GlobalChannelThermalDataset, batch: dict, device: torch.device, *, name: str, use_local: bool, mode: str) -> None:
    """Perform the run mode operation used by this module."""
