...
def synthetic_organizer() -> tuple[Dict[str, Any], np.ndarray]:
    """Perform the synthetic organizer operation used by this module."""
...
def permute_edges(aux: Dict[str, Any], perm: np.ndarray) -> Dict[str, Any]:
    """Perform the permute edges operation used by this module."""
...
def assert_same_plan(left: Dict[str, np.ndarray], right: Dict[str, np.ndarray], *, ignore_edge_permutation: bool = False) -> None:
    """Perform the assert same plan operation used by this module."""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Validate canonical HONF-CL hypergraph plan export.")
    parser.parse_args()
    ...
    with tempfile.TemporaryDirectory(prefix="honf_cl_plan_") as tmp:
