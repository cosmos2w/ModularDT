...
Run ``python src_HONF_CL/train_local.py --config <json>`` from the Demo 1
directory. The config selects standalone local data, global-alignment modules,
or a weighted mixture. One training-fitted normalizer is shared by all sources
and validation splits. Outputs are self-contained checkpoints, the resolved
config, a CSV loss history, and a loss curve.
"""
...
DEFAULT_CONFIG_PATH = "./src_HONF_CL/configs/train_local_module_template.json"
